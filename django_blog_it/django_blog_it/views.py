...
from .models import Menu, Post, PostHistory, Category, Tags, Image_File, \
    ...
    Post_Slugs
from .forms import AdminLoginForm, BlogPostForm, CustomBlogSlugInlineFormSet, BlogCategoryForm, UserForm, UserRoleForm, PageForm, MenuForm, ContactUsSettings, ContactUsSettingsForm, BlogThemeForm,ChangePasswordForm
...
def facebook_login(request):
    if 'code' in request.GET:
        accesstoken = Facebook.get_access_token_from_code(request.GET['code'], 'https://' + request.META['HTTP_HOST'] + reverse('facebook_login'), os.getenv("FB_APP_ID"), os.getenv("FB_SECRET"))
        if 'error' in accesstoken.keys():
            messages.error(request, "Sorry, Your session has been expired")
            return render(request, '404.html')
        graph = Facebook.GraphAPI(accesstoken['access_token'])
